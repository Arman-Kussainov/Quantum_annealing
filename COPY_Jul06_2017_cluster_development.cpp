		...
		if(id==ID){
			...
			for(int j2=0;j2<n;j2++){
				for(int i2=0;i2<m;i2++){
					...
					sc[i2][j2]=0;
					me_count++;}} // filling array with MAX value


			// applying energy stopping rules here
			int sites_count=0;
			hi_ts=p_cluster(i, j, m, n, s, sc, p, visited_added, neighbour,id, ID);
			int i_min=int(m/2),j_min=int(m/2),i_max=int(m/2),j_max=int(m/2);
			while(sites_count<=hi_ts){
				...
				if(abs(i_min-i_max)>=m-1){break;}						
				if(abs(j_min-j_max)>=n-1){break;}												
				sites_count++;} 
