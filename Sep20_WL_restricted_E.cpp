...
int main(int argc, char *argv[]){
	...
	while(c_ount<=total_steps){c_ount++;
	 //while(c_ount<=0){c_ount++;E2t=0;
		for (int ii=0;ii<10000*m*n;ii++){ // we assume that we should randomly access pretty much all spins in the array
		double E1t_old=E1t;
...
if(l_ock==1){			log_gE[int(E1t)+2*m*n]+=log(f);
			...
			if (r_atio>=0.95){
				...
				// normalize data
				int min_log_gE=max_H;
				for(int i=0;i<n_odes;i++){
					if(log_gE[i]!=0 && log_gE[i]<=min_log_gE){min_log_gE=log_gE[i];}}	
				for(int i=0;i<n_odes;i++){
					if(log_gE[i]!=0){log_gE[i]=log_gE[i]-min_log_gE;}}	
				...
				if(id==0){double s_hift=0;
					for(int j=0;j<ntasks;j++){double task_max=0;
						for(int i=0;i<n_odes;i++){
							if(log_gE_total[i+j*n_odes]>task_max)
								{task_max=log_gE_total[i+j*n_odes];}
							H_E[i]+=   H_E_total[i+j*n_odes];
							if(log_gE_total[i+j*n_odes]!=0){log_gE[i]+=(log_gE_total[i+j*n_odes]+s_hift);}}
							s_hift=s_hift+task_max;}
