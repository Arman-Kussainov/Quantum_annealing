...
int ID=0;
				...
				if(id==ID){
					...
					double E2_cluster=E_nergy(s, cos_theta, m, n, J);
					if((E_min<=E2_cluster/(m*n)) && (E2_cluster/(m*n)<E_max)){
						E2t=E2_cluster;}
					else{
						int sites_count=1; // still need to alter one point and test it later
						while(sites_count<=hi_ts){
