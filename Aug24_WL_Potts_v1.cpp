...
double dE(int *S[], double* cos_theta[], int m, int n, int i, int j, int nn, double J){
	...
	double DE = (cos_theta[S[i][j]][S[i+1][j]]+cos_theta[S[i][j]][S[i-1][j]]+cos_theta[S[i][j]][S[i][j+1]]+cos_theta[S[i][j]][S[i][j-1]])-                    
(cos_theta[nn][S[i+1][j]]+cos_theta[nn][S[i-1][j]]+cos_theta[nn][S[i][j+1]]+cos_theta[nn][S[i][j-1]]);
   return DE/2*J;}
...
int main(){
	...
	while(c_ount<=total_steps){c_ount++;
		for (int ii=0;ii<m*n;ii++){ // we assume that we should randomly access pretty much all spins in the array
			...
			//S[i][j]=o_ld;			
			d_E=dE(S,cos_theta, m+2, n+2, i, j, nn, J);
			E2t=E1t+d_E;
			//cout<<<<"\t"<<nn<<"\t"<<d_E<<"\n";
			...
			if (exp(log_gE[int(E1t)+2*m*n]-log_gE[int(E2t)+2*m*n])>=rand()/(RAND_MAX + 1.0)){
				...
				H_E[int(E1t)+2*m*n]+=1;} // no spin configuration update here. Reverse S[][] to s[][]
		}
		...
		if ((H_E[0]+H_E[4*m*n]>=2)&&(c_ount>=10000)&&(r_atio>=0.90)){
