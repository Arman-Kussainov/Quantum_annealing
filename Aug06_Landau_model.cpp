...
double e_nergy(double** ss, int mm, int nn){
//for(int i=0;i<mm;i++){for(int j=0;j<nn;j++){cout<<ss[i][i]<<"\t";}cout<<"\n";}
double s_um=0;
 for(int j=0;j<nn;j++){for(int i=0;i<mm;i++){s_um+=ss[i][j];}}
cout<<s_um<<"ffffffff";
}
...
int main(int argc, char *argv[]){
  ...
  int m=200, n=200;
  int id, ntasks;
  int total_steps=15000;  int therm_steps=int(0.2*total_steps);
  MPI_Init(&argc, &argv);
...
// this will clean the previous data stored in these files
   if(id==0){ofstream file("potts.txt");file.close();ofstream mfile("magnetization.txt");mfile.close();}
...
// the array will be expanded to provide threads with equal amount of data 
 ...
 for(int i = 0; i < m; i++)
    s[i] = s_data + n*i;
 // set element values such that cos(2*pi*s[i][j]/qq) = -1 for qq=2 ==> cos(pi), for qq>2 see for yourself..
double s_um=0;
    for(int j=0;j<n;j++){for(int i=0;i<m;i++){s[i][j] = qq-1;s_um+=s[i][j];}}
cout<<s_um<<"\n"<<"wwww";
//5555555555555555
e_nergy(s,m,n);
//5555555555555555
...
// calculate energy once and then only update it

          for (int ii=0;ii<m*RowsPerTask;ii++){ // we assume that we should randomly access pretty much all spins in the array at least once
	    int i = rand()%m+1; int j = rand()%RowsPerTask+1; // the indices should fall within the boundaries

            E1=J*(cos_theta[int(s_pins[i][j])][int(s_pins[i+1][j])]+cos_theta[int(s_pins[i][j])][int(s_pins[i-1][j])]
                 +cos_theta[int(s_pins[i][j])][int(s_pins[i][j+1])]+cos_theta[int(s_pins[i][j])][int(s_pins[i][j-1])]);
	    int nn=rand()%qq; // nn takes values from 0 to qq-1, because array indexed from 0 ==> cos(2pi*nn/qq)
            E2=J*(cos_theta[nn][int(s_pins[i+1][j])]+cos_theta[nn][int(s_pins[i-1][j])]
                 +cos_theta[nn][int(s_pins[i][j+1])]+cos_theta[nn][int(s_pins[i][j-1])]);
	    dE=E2-E1;
            if (exp(-dE/(k*T))>(rand()/(RAND_MAX + 1.0))){s_pins[i][j] = nn;

// here we need to calculate the energy...or update it...in order to speed up calculations

										}
         }

    double M_0=0;
    // s_pins[i][j]+1 because array indexed from 0 ==> cos(2pi*nn/qq)
    for(int i=0;i<m;i++){for(int j=0;j<RowsPerTask;j++){M_0+=cos(2*pi*(s_pins[i][j]+1)/qq);}} // spin projestions on y axis
	// we count the angle from Y axis as well!
 ...
 // to save magnetization configuration data every 50th step
//if ((c_ount%50)==0){if(c_ount>=therm_steps){cout<<c_ount<<"\t";
//  ofstream mfile("magnetization.txt",std::ofstream::out | std::ofstream::app); 
//     for(int q = 0; q < m; q++){
//        for(int t = 0; t < n; t++){
//            mfile<<cos(2*pi*(s[q][t]+1)/qq)<<"\t";}mfile<<"\n";}mfile.close();}}
...
} // loop "for(int c_ount=1;c_ount<=total_steps;c_ount++)" over monte carlo steps is over, that includes thermalization and magnetization stage
