...
int main(int argc, char *argv[]){
  ...
  int m=8, n=8;
  int id, ntasks;
  int total_steps=2100;  int therm_steps=int(0.2*total_steps);
  MPI_Init(&argc, &argv);
 ...
 // now we need to simulate periodic boundary by additional data column for each process
// int M=m+2; // WARNING NOTATION two additional raws at the top and bottom
...
//  for(int j=0;j<n;j++){for(int i=0;i<m;i++){s[i][j] = c_ount;c_ount++;}}
...
//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*
  ...
  for (int i = 0; i < (m+2); ++i)
   S[i] = S_data + N*i;
    for(int i=0;i<(m+2);i++){for(int j=0;j<N;j++){S[i][j] = 1;}}
 ...
 // display data for control
// if(id==0){  
// for(int i=0;i<m;i++){for(int j=0;j<n;j++){cout<<round(s[i][j])<<"\t";}cout<<"\n";}}
// cout<<"\n";
// for(int i=0;i<(m+2);i++){for(int j=0;j<N;j++){cout<<round(S[i][j])<<"\t";}cout<<"\n";}}


// back reconstruction of the expanded array
//  cou_nt=0;
//  for(int p_os=1;p_os<=1+(ntasks-1)*(RowsPerTask+2);p_os+=(RowsPerTask+2)){
//   for(int j=0+p_os;j<RowsPerTask+p_os;j++){
//    for(int i=1;i<(m+2)-1;i++){
//      s[i-1][j-1-2*cou_nt]=S[i][j];}}cou_nt++;}

// if(id==0){ 
// cout<<"\n"; 
// for(int i=0;i<m;i++){for(int j=0;j<n;j++){cout<<s[i][j]<<"\t";}cout<<"\n";}}
...
//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*//*
  ...
  for(double T=dT;T<=3;T+=dT){
      ...
      for(int c_ount=1;c_ount<=total_steps;c_ount++){
         ...
         double M_0=0;
         for(int i=1;i<m+1;i++){for(int j=1;j<RowsPerTask+1;j++){M_0+=s_pins[i][j];}}
