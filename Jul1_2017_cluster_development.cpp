...
/****************************************************************************************************************************/
void p_cluster(int i, int j, int m, int n, int *s[],double *p[],int *visited_added[], int neighbour[2][4]){
...
int next_start=0; int next_end=0;
int h_its=0;

int old_counts=h_its,
	new_counts=h_its+1;
	...
	for (int l=next_start;l<=next_end;l++){
		...
		for(int io=0; io<=3;io++){
			if((neighbour[0][io]!=9999)&&(p[neighbour[0][io]][neighbour[1][io]]<m_in)){
				...
				index_i=neighbour[0][io];index_j=neighbour[1][io];}}
	}
...
int main(int argc, char *argv[]){
	...
	double l_ow=-2, u_pper=-0.01, l_ength=1.2*((u_pper-l_ow)/ntasks); // 1.01 just to give a slight overlap
...
//*******************************************************************************************************
	...
	// create COPY of the main, basic 2D array of spins
...
//*******************************************************************************************************
	...
	while((E1t/(m*n)<E_min)||(E1t/(m*n)>E_max)){ // very inefficent. need to push it to the right boundaris
		...
		// reassign random values... may be not to the whole array? but only to visited? or the cluster?
		srand (time(NULL)+id);	
		for(int j=0;j<n;j++){
			for(int i=0;i<m;i++){
		...
		//get_cluster(i, j, m, n, s, visited_added, neighbour,id);
		//p_cluster(i, j, m, n, s, p,visited_added, neighbour,id);
	...
	while(c_ount<=total_steps){c_ount++;
		...
		for (int ii=0;ii<m*n;ii++){			
			...
			p_cluster(i, j, m, n, s, p, visited_added, neighbour);
			// applying energy stopping rules here
			int stop_flag=0, sites_count=0;
			int nn_old;
			float E2t;
			while(stop_flag!=1){
				i=visited_added[0][sites_count];
				j=visited_added[1][sites_count];
				if(abs(visited_added[0][sites_count])>=m){break;}
