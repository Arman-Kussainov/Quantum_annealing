...
int main(){
	...
	for(int j=0;j<n;j++){
		for(int i=0;i<m;i++){
			s[i][j] = qq-1;}} // is q-1 the max value of n?
	...
	//while(c_ount<=total_steps){c_ount++;
	 while(c_ount<=0){c_ount++;E2t=0;
		for (int ii=0;ii<140*m*n;ii++){ // we assume that we should randomly access pretty much all spins in the array
			...
			int nn_old=S[i][j];	cout<<S[i][j]<<"\t";S[i][j]=nn; cout<<S[i][j]<<"\t"	;
			d_E=dE(S,cos_theta,i,j,nn_old,J);cout<<d_E<<"\t";
			E2t=E1t+d_E;
			//cout<<ii<<"\t"<<E1t<<"\t"<<E2t<<"\n";
			...
			if (exp(log_gE[int(E1t)+2*m*n]-log_gE[int(E2t)+2*m*n])>=rand()/(RAND_MAX + 1.0)){
				...
				H_E[int(E2t)+2*m*n]+=1;
				s[i-1][j-1] = nn; // update original matrix
				cout<<"*"<<S[i][j]<<"*"<<"\n";
