...
int main(int argc, char *argv[]){
 ...
 // create collective 2D array of spins
  ...
  for(int i=0;i<m;i++){for(int j=0;j<n;j++){s[i][j] = 1.0;}}
   for(int j=0;j<n;j++){s[0][j]=3;s[2][j]=4;}
   for(int j=0;j<n;j++){s[m-1][j]=5;s[m-2][j]=6;}
...
if(id==0){ofstream file("ising.txt");file.close();
  ...
  int total_steps=150;  int therm_steps=int(0.2*total_steps);
  double dT=0.02;
  ...
  for (int t = 0; t < RowsPerTask; t++){
   for (int q = 0; q < m; q++){
    s_recv[q * RowsPerTask + t] = s_pins[q][t];
}}
