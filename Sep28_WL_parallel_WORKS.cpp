...
int main(int argc, char *argv[]){
	...
	while(c_ount<=total_steps){c_ount++;
		for (int ii=0;ii<100*m*n;ii++){ // we assume that we should randomly access pretty much all spins in the array
			double E1t_old=E1t;
...
//-----------------------------------------------------------------------------
				...
				if((l_ock==0)&&(old_lock==1)){// walker just crossed the border between processes
					s[i-1][j-1]=nn_old;S[i][j]=nn_old; E1t=E1t_old; l_ock=1;// force walker to come back FROM BOTH SIDES
					...
					if((i==1)||(i==m)||(j==1)||(j==n)){// update the big matrix boundaries
						...
						for(int i=0;i<(m+2);i++){S[i][0]=S[i][n];S[i][n+1]=S[i][1];}}}
//-----------------------------------------------------------------------------					
			}else{ // walker stays. still need to check if it is in the propere energy region
				if(l_ock==1){
