...
void four_pattern(int i, int j, int m, int n, int *s[], int *visited_added[], int neighbour[2][4]){
	...
	for(int i1=i-1;i1<=i+1;i1++){
		for(int j1=j-1;j1<=j+1;j1++){
			...
			if ((abs(j1+i1-j-i)==1) && (i1>=0) && (i1<m) && (j1>=0) && (j1<n)){
					...
					int f_ound=0;int s_can=0;
					for(int s_can=0;s_can<m*n;s_can++){
						if((visited_added[0][s_can]==i1)&&(visited_added[1][s_can]==j1)){f_ound=1;}}
						
					if((s[i1][j1]==s[i][j])&&(f_ound!=1)){
...
visited_added[0][0]=i; visited_added[1][0]=j;
...
while(next_start<=next_end){
	...
	for (int l=next_start;l<=next_end;l++){
		...
		//cout<<neighbour[0][0]<<" "<<neighbour[1][0]<<"\n";
		//cout<<"+";
		four_pattern(ic, jc, m, n, s, visited_added, neighbour);
		//cout<<"+";
		//cout<<"\n"<<neighbour[0][0]<<" "<<neighbour[1][0]<<"\n";			
			
		
		for(int io=0; io<=3;io++){
			if((neighbour[0][io]!=9999)&&(neighbour[1][io]!=9999)){
				h_its=h_its+1;
                s_hift=s_hift+1;
