...
double dE(int *s[], double* cos_theta[], int m, int n, int i, int j, double J){
    double s_um=0;
    int up,down,left,right;
    ...
    return s_um*J;} // in dE need to remove division by 2...for a single spin
...
//	cout<<"**"<<id<<"\t"<<E_min<<"\t"<<E_max<<E1t<<"\t";
	...
	while(c_ount<=total_steps){c_ount++;
		...
		float d1=dE(s, cos_theta, m, n, i,j,J);
		for (int ii=0;ii<9*m*n;ii++){ // we assume that we should randomly access pretty much all spins in the array
