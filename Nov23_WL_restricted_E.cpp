...
int main(int argc, char *argv[]){
	...
	double over_lap=0.5*(u_pper-l_ow)/ntasks;
	double E_min=l_ow+double(id)*(u_pper-l_ow)/ntasks,
	       E_max=l_ow+double(id+1)*(u_pper-l_ow)/ntasks+over_lap;
	...
	// specifically designed for q=13
...
//			s[i][j] = rand()%qq;
	...
	// 2D array of the precomputed cos(theta_j-theta_i) or delta function values
...
//---------------------------------------------------------------------------------------------				
				if(id==0){
					ofstream dfile("long_data.txt");
					for(int i=0;i<n_odes*ntasks;i++){
						dfile<<log_gE_total[i]<<"\t"<<H_E_total[i]<<"\n";}
					dfile.close();
					break;
