...
int main(int argc, char *argv[]){
	...
	// specifaclly designed for q=7,10
	if(id>=int(ntasks/2)+2){
		for(int j=0;j<n;j++){
			for(int i=0;i<m;i++){
//			s[i][j] = qq-1;
			s[i][j] = rand()%qq;
...
// to test energy assignment procedure
	if(id==7){

		ofstream Efile("E_test.txt");
		...
		double E_test=e_nergy(S, cos_theta, m+2, n+2, J);
		//cout<<jj*m+ii<<"\t"<<E_test/(m*n)<<"\n";
		//Efile<<jj*m+ii<<"\t"<<E_test/(m*n)<<"\n";
							}}
