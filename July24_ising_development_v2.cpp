...
// the array will be expanded to provide threads with equal amount of data 
 ...
 // for(int j=0;j<n;j++){for(int i=0;i<m;i++){s[i][j] = +1;}}
    for(int j=0;j<n;j++){for(int i=0;i<m;i++){int nn=rand()%qq; s[i][j] = cos(2*pi*double(nn)/double(qq));}}
   ...
   for (int t = 0; t < RowsPerTask+2; t++){
    for (int q = 0; q < m+2; q++){
     s_pins[q][t] = s_recv[t * (m+2) + q];}}
        ...
        for (int ii=0;ii<8*m*RowsPerTask;ii++){ // we assume that we should randomly access pretty much all spins in the array
	...
	//*********
	  // First we will update top and bottom boundaries within the cluster
	  // do not need communicator for these boundaries
	//  if(i==1){s_pins[m+1][j]=s_pins[i][j];} else if(i==m){s_pins[0][j]=s_pins[i][j];}
	//*********
	 // Now left and right boundaries
	// if(j==1){}
	//if(j==RowsPerTask){}
	// ... I need to make sure that time flows equally fast in all compartment
	// otherwise, what's the point of different compartments cooling down faster and delivering their boundary info to another compartments
	}
