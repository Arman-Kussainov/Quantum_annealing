...
// 2D array of the cos(theta_j-theta_i) values
 ...
 // set element values = 1
    for(int j=0;j<n;j++){for(int i=0;i<m;i++){nn=rand()%qq+1; s[i][j] = 2*pi*double(nn)/double(qq);}}
//  for(int j=0;j<n;j++){for(int i=0;i<m;i++){nn=rand()%qq+1; s[i][j] = qq;}}//2*pi*double(nn)/double(qq);}}
