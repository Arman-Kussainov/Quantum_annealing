...
int main(int argc, char *argv[]){
	...
	while(c_ount<=total_steps){c_ount++;
		for (int ii=0;ii<2*m*n;ii++){ // we assume that we should randomly access pretty much all spins in the array
...
//-----------------------------------------------------------------------------											
			...
			if(r_atio>=0.90){
				...
				if(id==0){
					for(int i=0;i<n_odes;i++){loggE_dis[i]=0;}
...
//	double* p_artition = new double[ntasks+1];
//	p_artition[ntasks]=u_pper;
//	double d_ivider=1;//cout<<p_artition[ntasks]<<"\t";
//	for(int i=ntasks-1;i>0;i--){
//		d_ivider*=2;
//		p_artition[i]=p_artition[i+1]-(u_pper-l_ow)/d_ivider;}
//		p_artition[0]=l_ow;
//	E_min=p_artition[id], E_max=p_artition[id+1];
//	cout<<id<<"\t"<<E_min<<"\t"<<E_max<<"\n";
