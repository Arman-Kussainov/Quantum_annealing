...
/****************************************************************************************************************************/
int p_cluster(int i, int j, int m, int n, int *s[],int *sc[],double *p[],int *visited_added[], int neighbour[2][4], int id, int I_D){
	...
	if(id==I_D){
		...
		while(old_counts!=new_counts){
			...
			if(m_in!=1){
				...
				next_end=next_end+1;
				new_counts++;} //if((abs(i_min-i_max)>=63)||(abs(j_min-j_max)>=63)){break;}
		}
	}
	return h_its;
}
...
//*******************************************************************************************************
	...
	if(id==0){
		ofstream cl_uster("cluster_0.txt");

		for(int j=0;j<n;j++){
			for(int i=0;i<m;i++){ 
				if(i==0&&j==0){cl_uster<<E1t/(m*n)<<"\t";}else{
				cl_uster<<s[i][j]<<"\t";}}
				cout<<"\n";}
				cl_uster.close(); cout<<"done";}	
		...
		srand (time(NULL)+id);
		int c_ount=0;
		// we start with E1t and make step to E2t
		float E2t;
...
//int c_size=1; // 1 means the single point
			...
			int nn=s[is][js]; // save the old spin value of the site
			while(nn==s[is][js]){ // need for sure another value... yes..otherwise no flip is observed
			nn =rand()%qq;} // nn takes values from 0 to qq-1, because array indexed from 0 ==> cos(2pi*nn/qq)....	
			
			int nn_old=s[is][js];
			
			// too slow...refresh some values
			int me_count=0;
			...
			int h_its=p_cluster(is, js, m, n, s, sc, p, visited_added, neighbour,id, ID);
			cout<<"<<<<<<<<"<<h_its<<"<<<<<<<<<<<<"<<"\t";
			int sites_count=0;
			E2t=E1t;		
			//cout<<E_min<<"\t"<<E2t/(m*n)<<"\t"<<E_max<<"\t"<<sites_count<<"\t"<<h_its<<"\n";	
			...
			while(sites_count<=h_its){
				...
				if(E2t/(m*n)>E_max){cout<<"%%%%%%%%%%%%%%"<<E2t/(m*n)<<"\n";break;}

				// WITHOUT THIS RULE EVERYTING IS GARBAGE ))
				//if(p[iw][jw] <= rand()/(RAND_MAX + 1.0)){break;}				
...
//			cout<<sites_count<<"\n";
			 ...
			 E2t=E_nergy(s, cos_theta, m, n, J);
			//cout<<E_min<<"\t"<<E2t/(m*n)<<"\t"<<E_max<<"\n";	



			
	if(id==0){
		ofstream c_luster("cluster.txt");

		for(int j=0;j<n;j++){
			for(int i=0;i<m;i++){ 
				if(i==0&&j==0){c_luster<<E2t/(m*n)<<"\t";}else{
				c_luster<<s[i][j]<<"\t";}}
				cout<<"\n";}
				c_luster.close(); cout<<"DONE!";}	
				...
				while(nn==s[i][j]){
				nn =rand()%qq;
				} // nn takes values from 0 to qq-1, because array indexed from 0 ==> cos(2pi*nn/qq)....	
	...
	break;	}break; //HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH
