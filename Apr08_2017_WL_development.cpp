...
int main(int argc, char *argv[]){
	...
	double l_ow=-2, u_pper=-0.1, l_ength=1.1*((u_pper-l_ow)/ntasks); // 1.01 just to give a slight overlap
...
//	cout<<"**"<<id<<"\t"<<E_min<<"\t"<<E_max<<E1t<<"\t";
	...
	while((E1t/(m*n)<E_min)||(E1t/(m*n)>E_max)){ // very inefficent. need to push it to the right boundaris
		...
		int nn=s[i][j]; // save the old spin value of the site
		while(nn==s[i][j]){ // need for sure another value... yes..otherwise no flip is observed
