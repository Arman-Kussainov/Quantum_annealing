...
double dE(int *S[],int *S0[],int *s[], double* cos_theta[],
		 int m, int n, int i, int j, int nn_old, double J){

double DE0=0,DE1=0,DE2=0,DE3=0,DE4=0;

if(i==1){DE1=(  cos_theta[S[m-2][j]][S[m-2+1][j]]
			   +cos_theta[S[m-2][j]][S[m-2-1][j]]
			   +cos_theta[S[m-2][j]][S[m-2][j+1]]
			   +cos_theta[S[m-2][j]][S[m-2][j-1]]
			   
			   +cos_theta[S[i][j-1]][S[i+1][j-1]]
			   +cos_theta[S[i][j-1]][S[i-1][j-1]]
			   +cos_theta[S[i][j-1]][S[i][j+1-1]]
			   +cos_theta[S[i][j-1]][S[i][j-1-1]]
			   ...
			   (cos_theta[S0[m-2][j]][S0[m-2+1][j]]
			   +cos_theta[S0[m-2][j]][S0[m-2-1][j]]
			   +cos_theta[S0[m-2][j]][S0[m-2][j+1]]
			   +cos_theta[S0[m-2][j]][S0[m-2][j-1]]);}
			   
			   
if(i==m-2){DE2=(cos_theta[S[1][j]][S[1+1][j]]
			   +cos_theta[S[1][j]][S[1-1][j]]
			   +cos_theta[S[1][j]][S[1][j+1]]
			   +cos_theta[S[1][j]][S[1][j-1]])-
			   
			   (cos_theta[S0[1][j]][S0[1+1][j]]
			   +cos_theta[S0[1][j]][S0[1-1][j]]
			   +cos_theta[S0[1][j]][S0[1][j+1]]
			   +cos_theta[S0[1][j]][S0[1][j-1]]);}			   
	
if(j==1){DE3=(  cos_theta[S[i][n-2]][S[i+1][n-2]]
			   +cos_theta[S[i][n-2]][S[i-1][n-2]]
			   +cos_theta[S[i][n-2]][S[i][n-2+1]]
			   +cos_theta[S[i][n-2]][S[i][n-2-1]])-
			   
			   (cos_theta[S0[i][n-2]][S0[i+1][n-2]]
			   +cos_theta[S0[i][n-2]][S0[i-1][n-2]]
			   +cos_theta[S0[i][n-2]][S0[i][n-2+1]]
			   +cos_theta[S0[i][n-2]][S0[i][n-2-1]]);}

if(j==n-2){DE4= (cos_theta[S[i][1]][S[i+1][1]]
			   +cos_theta[S[i][1]][S[i-1][1]]
			   +cos_theta[S[i][1]][S[i][1+1]]
			   +cos_theta[S[i][1]][S[i][1-1]])-
			   
			   (cos_theta[S0[i][1]][S0[i+1][1]]
			   +cos_theta[S0[i][1]][S0[i-1][1]]
			   +cos_theta[S0[i][1]][S0[i][1+1]]
			   +cos_theta[S0[i][1]][S0[i][1-1]]);}

		  DE0 =(cos_theta[S[i][j]][S[i+1][j]]
			   +cos_theta[S[i][j]][S[i-1][j]]
			   +cos_theta[S[i][j]][S[i][j+1]]
			   +cos_theta[S[i][j]][S[i][j-1]])-
			   
			   (cos_theta[nn_old][S0[i+1][j]]
			   +cos_theta[nn_old][S0[i-1][j]]
			   +cos_theta[nn_old][S0[i][j+1]]
			   +cos_theta[nn_old][S0[i][j-1]]);
			   
			   cout<<"\n";
			   for(int i=0;i<=m-1;i++){for(int j=0;j<=n-1;j++){cout<<S[i][j]<<"*\t";}cout<<"\n";}
			   
  			   cout<<"\n";
			   for(int i=0;i<=m-1;i++){for(int j=0;j<=n-1;j++){cout<<S0[i][j]<<"\t";}cout<<"\n";}

			   
cout<<"\n**"<<DE0<<"\t"<<DE1<<"\t"<<DE2<<"\t"<<DE3<<"\t"<<DE4<<"**\n";

   double DE=(DE0+DE1+DE2+DE3+DE4);
   return DE/2*J;}
...
int main(){
	...
	//  This array will be holding the replica of the S[][] data
	int* S0_data = new int[(m+2)*(n+2)];
	int** S0 = new int*[(m+2)];
	for(int i = 0; i < (m+2); ++i){
		S0[i] = S0_data + (n+2)*i;}
	for(int i=0;i<(m+2);i++){
...
cout<<"\n"<<ii<<"\n";
			...
			if((i==1)||(i==m)||(j==1)||(j==n)){// update the big matrix boundaries
			 ...
			 for(int i=0;i<(m+2);i++){S[i][0]=S[i][n];S[i][n+1]=S[i][1];}}
			d_E=dE(S,S0,s,cos_theta, m+2, n+2,i,j, nn_old,J);
			E2t=E1t+d_E;			
			...
			// update boundaries in order to use later the functions e_nergy and d_E!!
			// and everything else
			S0[i][j]=nn;
			if((i==1)||(i==m)||(j==1)||(j==n)){// update the big matrix boundaries
			 // top and bottom boundaries update
			 for(int j=0;j<(n+2);j++){S0[0][j]=S[m][j];S0[m+1][j]=S[1][j];}
			 // left and right boundaries update
			 for(int i=0;i<(m+2);i++){S0[i][0]=S[i][n];S0[i][n+1]=S[i][1];}}				
				...
				S[i][j]=nn_old;
				// reverse update boundaries in order to use functions e_nergy and d_E!!
				if((i==1)||(i==m)||(j==1)||(j==n)){// update the big matrix boundaries
				 // top and bottom boundaries update
				 for(int j=0;j<(n+2);j++){S[0][j]=S[m][j];S[m+1][j]=S[1][j];}
				 // left and right boundaries update
